...
async fn i6_pac_endpoint_unauthenticated() {
    ...
    assert!(response.contains("FindProxyForURL"), "Missing PAC function");
}
...
async fn i7_pac_content_correctness() {
    ...
    assert!(
        ...
        "PAC missing exact match for github.com: {}",
        body
    );
    assert!(
        body.contains("dnsDomainIs(host, \".crates.io\")"),
        "PAC missing wildcard for *.crates.io: {}",
        body
    );
    assert!(
...
async fn i8_non_pac_plaintext_request_denied() {
    ...
    assert!(
        response.contains("tls_required"),
        "Missing tls_required body: {}",
        response
    );
}
